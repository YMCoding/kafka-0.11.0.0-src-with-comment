...
//一个broker上可能分布多个partition信息，manager主要是管理一个broker上的partition
class ReplicaManager(val config: KafkaConfig,
                     metrics: Metrics,
                     time: Time,
                     val zkUtils: ZkUtils, //操作zk的工具类
                     scheduler: Scheduler, //用于执行ReplicaManager中的定时任务
                     val logManager: LogManager, //对分区的读写操作都委托给底层的日志存储系统
                     val isShuttingDown: AtomicBoolean,
                     quotaManager: ReplicationQuotaManager,
                     val brokerTopicStats: BrokerTopicStats,
                     val metadataCache: MetadataCache,
                     val delayedProducePurgatory: DelayedOperationPurgatory[DelayedProduce],//用来管理DelayedProduce
                     val delayedFetchPurgatory: DelayedOperationPurgatory[DelayedFetch], //用来管理DelayedFetch
  ...
  //记录KafkaController年代信息，重新选举leader后，会递增
  @volatile var controllerEpoch: Int = KafkaController.InitialControllerEpoch - 1

  //当前broker id
  private val localBrokerId = config.brokerId

  //保存了当前Broker上分配的所有的Partition信息
  ...
  //管理多个ReplicaFetcherThread线程，向leader副本发送FetchRequest请求来获取信息
  val replicaFetcherManager = createReplicaFetcherManager(metrics, time, threadNamePrefix, quotaManager)

  private val highWatermarkCheckPointThreadStarted = new AtomicBoolean(false)

  //用于缓存log与offsetCheckpoint之间的对应关系，offsetCheckpoint记录了log目录下replication-offset-checkpint文件
  ...
  //hightwatermark-checkpoint未启动，则启动
  //hightwatermark-checkpoint任务会周期性的记录每个Replica的HW并保存到其log目录下的replication-offset-checkpoint文件中
  ...
  def maybePropagateIsrChanges() {
    ...
    isrChangeSet synchronized {
      if (isrChangeSet.nonEmpty && //不为空
        (lastIsrChangeMs.get() + ReplicaManager.IsrChangePropagationBlackOut < now || //最后一次isr集合发生变化的时间距今已超过5秒
          lastIsrPropagationMs.get() + ReplicaManager.IsrChangePropagationInterval < now)) { //最后一次写入zk的时间距离今天已经超过60秒
        //将isrChangeSet写入zk
  ...
  def startup() {
    ...
    //启动isr-expiration定时任务
    //isr-expiration任务会周期性的调用maybeShrinkIsr检测每个分区是否需要缩减其isr集合
    scheduler.schedule("isr-expiration", maybeShrinkIsr _, period = config.replicaLagTimeMaxMs / 2, unit = TimeUnit.MILLISECONDS)
    //启动isr-change-propagation任务
    //定期的间isr集合发生变化的分区记录到Zk中
  ...
  def stopReplica(topicPartition: TopicPartition, deletePartition: Boolean): Errors  = {
    ...
    getPartition(topicPartition) match {
      case Some(_) =>
        //只有当deletePartition为true时，才会真正删除该分区对应的副本及其logz
        if (deletePartition) {
          ...
          if (removedPartition != null) {
            ...
            //如果allPartions中不存在对应的分区对象，直接尝试删除log
  ...
  def appendRecords(timeout: Long,
    ...
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      if (delayedProduceRequestRequired(requiredAcks, entriesPerPartition, localProduceResults)) {
        ...
        //尝试将完成DelayedProduce，否则将DelayedProduce添加到delayedProducePurgatory中管理
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
                               isFromClient: Boolean,
                               entriesPerPartition: Map[TopicPartition, MemoryRecords],//每个分区对应的消息
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      //检测topic是否是内部topic，如果是内部topic则根据internalTopicsAllowed决定是否向内部topic写入消息。
  ...
  def readFromLocalLog(replicaId: Int,
    ...
    def read(tp: TopicPartition, fetchInfo: PartitionData, limitBytes: Int, minOneMessage: Boolean): LogReadResult = {
      ...
      try {
        ...
        //获取要读取消息得副本，根据fetchOnlyFromLeader来判断是不是leader副本
        ...
        //确认读取消息得offset上限，根据readOnlyCommitted来判断是不是hw
        ...
        //是FectchDataInfo对象，其中包含LogOffsetMetadata和messgeset
  ...
  //获取指定的Partition对象
  //根据parttionStates信息对其进Leader/Follower切换
  //启动highwatermart-checkpoint任务
  def becomeLeaderOrFollower(correlationId: Int,leaderAndISRRequest: LeaderAndIsrRequest,
    ...
    replicaStateChangeLock synchronized {
      ...
      if (leaderAndISRRequest.controllerEpoch < controllerEpoch) {
        ...
        //管理ReplicaFetcherManager中空闲的Fetche线程
  ...
  private def makeLeaders(controllerId: Int,
    ...
    try {
      ...
      //此Broker上的副本之前可能是Follower，所以暂停对这些副本的fetch操作
      replicaFetcherManager.removeFetcherForPartitions(partitionState.keySet.map(_.topicPartition))
      // Update the partition information to be the leader
      partitionState.foreach{ case (partition, partitionStateInfo) =>
        //将分区的local replica切换成Leader副本
        if (partition.makeLeader(controllerId, partitionStateInfo, correlationId))
          //记录成功从其他状态切换成leader副本对应的分区
   ...
   */
  //将local replica切换成followe副本
  //从leader-follower需要检测leader是否存活
  private def makeFollowers(controllerId: Int,
    ...
    try {
      ...
      partitionState.foreach{ case (partition, partitionStateInfo) =>
        ...
        //检测leader所在的brokerid是否存活
        val newLeaderBrokerId = partitionStateInfo.leader
        metadataCache.getAliveBrokers.find(_.id == newLeaderBrokerId) match {
          // Only change partition state when the leader is available
          case Some(_) =>

            //将分区的local replica切换成follower副本
          ...
          case None =>
            ...
            //即使是leader副本不能用。也要创建local replica
  ...
  //更新Leader副本上维护的follower副本的状态
  //随着Follower副本不断fetch消息，最终追上leader副本，isr可能会扩张
  //检测是否需要后移leader的hw
  //检测delayedProducePurgatory中相关的key对应的DelayedProduce。满足条件则将其执行完成
  private def updateFollowerLogReadResults(replicaId: Int, readResults: Seq[(TopicPartition, LogReadResult)]) {
    ...
    readResults.foreach { case (topicPartition, readResult) =>
      getPartition(topicPartition) match {
        case Some(partition) =>
          //更新follower副本的状态调用mybeExpandIsr进行扩张
