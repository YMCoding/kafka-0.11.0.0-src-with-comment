 ...
 */
//用来缓存整个集群中全部分区状态的组件
//KafkaController通过向集群中的Broker发送UpdateMetadatRequest来更新其MetadataCachce中缓存的数据
//每个Broker在收到该请求后会异步更新MetadataCache中的数据
class MetadataCache(brokerId: Int) extends Logging {
  private val stateChangeLogger = KafkaController.stateChangeLogger
  private val cache = mutable.Map[String, mutable.Map[Int, PartitionStateInfo]]() //记录每个分区的状态，PartitionStateInfo记录partition的状态
  private var controllerId: Option[Int] = None
  private val aliveBrokers = mutable.Map[Int, Broker]() //记录了当前可以用broker信息。
  private val aliveNodes = mutable.Map[Int, collection.Map[ListenerName, Node]]() //记录了可用节点的信息
  ...
  private def getPartitionMetadata(topic: String, listenerName: ListenerName, errorUnavailableEndpoints: Boolean): Option[Iterable[MetadataResponse.PartitionMetadata]] = {
    ...
    cache.get(topic).map { partitions =>
      partitions.map { case (partitionId, partitionState) =>
        ...
        //获取分区的leaderAndIsr对象，其中记录了leaderId，leaderEpoch，Isr集合以及controllerEpoch
        ...
        maybeLeader match {
          ...
          case Some(leader) =>
            ...
            } else if (isrInfo.size < isr.size) {//检测isr集合中的副本是否是可用的
  ...
  def getTopicMetadata(topics: Set[String], listenerName: ListenerName, errorUnavailableEndpoints: Boolean = false): Seq[MetadataResponse.TopicMetadata] = {
    inReadLock(partitionMetadataLock) {
      topics.toSeq.flatMap { topic =>
        //调用getPartitionMetadata方法获取PartitonMetadata。并将PartitionMetadata与topic信息进行封装
  ...
  def updateCache(correlationId: Int, updateMetadataRequest: UpdateMetadataRequest): Seq[TopicPartition] = {
    ...
    inWriteLock(partitionMetadataLock) {
      ...
      //将aliveNodes和aliveBroker中信息中清除掉
      aliveNodes.clear()
      aliveBrokers.clear()
      //交live_brokers字段重新构建
