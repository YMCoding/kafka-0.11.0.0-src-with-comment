...
class KafkaApis(val requestChannel: RequestChannel,
   ...
   */
  //负责将handler传递过了来的请求发送到不同的handler方法中
  ...
  // 都某个leader副本所在的broker上运行的broker出现故障时候
  // comsumer group则有新的leader副本所在的broker上运行的GroupCoordinator负责管理
  // 由此方法将gropCache和offsetsCache集中中的信息转义到新GroupCoodirnator上
  def handleLeaderAndIsrRequest(request: RequestChannel.Request) {
    ...
    try {
      def onLeadershipChange(updatedLeaders: Iterable[Partition], updatedFollowers: Iterable[Partition]) {
        ...
        updatedLeaders.foreach { partition =>
          if (partition.topic == GROUP_METADATA_TOPIC_NAME)
            // 当broker成为offset topic分区的leader副本后，会回调handleGroupImmigration
        ...
        updatedFollowers.foreach { partition =>
          if (partition.topic == GROUP_METADATA_TOPIC_NAME)
            // 当broker成为Offsets topic分区的follower副本会回调handleGroupEmigration 方法进行清除工作
      ...
      if (authorize(request.session, ClusterAction, Resource.ClusterResource)) {
        // 将onLeadershipChange作为方法的回调函数传入
      ...
      if (authorizedTopics.isEmpty)
        ...
        val offsetRetention =
          if (header.apiVersion <= 1 ||
            ...
            groupCoordinator.offsetConfig.offsetsRetentionMs // 默认是24小时
          else
        ...
        // 根据配置保留时间，或者每个分区指定的保留时间，计算出offset的过期清理时间
        ...
        // 将请求携带的信息和offsetandmetadata对象交给handleCommitOffsets方法处理
  ...
  def handleTopicMetadataRequest(request: RequestChannel.Request) {
    ...
    val topics =
      //请求的topic集合字段为空集合，则读取所有的Topic信息
   ...
   */
  // 当Comsumer group宕机后重新上线，可以通过向GroupCoordinator发送OffsetFetchRequest获取最近一次提交的offset,从此位置重新开始进行消费
  def handleOffsetFetchRequest(request: RequestChannel.Request) {
    ...
    // 根据OffsetFetchRequest的版本号来进行不同的读取offset的流程处理
    // apiversion 为0的时候使用旧版请求，此时offset存储在zk中
    // apiversion 为1的时候使用新版本请求，此时提交的offset交个GroupCoordinator管理
      ...
      val offsetFetchResponse =
        ...
        if (!authorize(request.session, Read, new Resource(Group, offsetFetchRequest.groupId)))
          ...
          if (header.apiVersion == 0) {
            ...
            // 将OffsetFetchRequest请求委托给GroupCoordinator处理
  ...
  def handleJoinGroupRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(request.session, Read, new Resource(Group, joinGroupRequest.groupId()))) {
      ...
      // 将JoinGroupRequest交给handleJoinGroup处理
  ...
  def handleHeartbeatRequest(request: RequestChannel.Request) {
    ...
    if (!authorize(request.session, Read, new Resource(Group, heartbeatRequest.groupId))) {
      ...
      // 将heartbeatrequest委托给groupCordinator处理
  ...
  // 消费者离开group 时候会取消对topic的订阅，发送LeaveGroupRequest请求
  ...
  def ensureInterBrokerVersion(version: ApiVersion): Unit = {
    if (config.interBrokerProtocolVersion < version)
      throw new UnsupportedVersionException(s"inter.broker.protocol.version: ${config.interBrokerProtocolVersion.ve  rsion} is less than the required version: ${version.version}")
  }
