...
class ReplicaFetcherThread(name: String,
                           ...
                           leaderEndpointBlockingSend: Option[BlockingSend] = None)//BlockingSend中封装了NetworkClientUtils
   ...
   */
  //处理follower副本请求offset超出了leader副本的offset范围
  def handleOffsetOutOfRange(topicPartition: TopicPartition): Long = {
     ...
     */
    //发送ListOffsetRequest获取leader副本的leo,使用阻塞方式
    ...
    if (leaderEndOffset < replica.logEndOffset.messageOffset) {
      ...
      //lader中第一条消息得offset大于follower副本的leo
      ...
      //将分区对应的log截断到leader副本的leo位置，之后从此offset开始重新与leader进行同步
