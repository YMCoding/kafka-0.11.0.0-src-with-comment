...
abstract class DelayedOperation(override val delayMs: Long) extends TimerTask with Logging {
   ...
   */
  //如果DelayedOperation没有完成，则将任务从时间轮中删除掉，在执行onComplete方法
...
final class DelayedOperationPurgatory[T <: DelayedOperation](purgatoryName: String,
  ...
  //推进时间轮
  //定期清理watcherForKey中完成的DelayedOperation
   ...
   */
  //检测DelayedOperation是否完成，为完成则添加到watchersForKey已经SystemTimer中
  def tryCompleteElseWatch(operation: T, watchKeys: Seq[Any]): Boolean = {
    ...
    //传入的key表示DelayedOperation关系的条件
    for(key <- watchKeys) {
      // If the operation is already completed, stop adding it to the rest of the watcher list.
      //添加过程中若已经被其他线程完成，则放弃后续添加过程
      if (operation.isCompleted)
        return false
      //讲DelayedOperation添加到watchersForKey中对应的Watcher中
      watchForOperation(key, operation)

      if (!watchCreated) {
        watchCreated = true
        //增减estimatedTotalOperations的值
        estimatedTotalOperations.incrementAndGet()
      }
    }
    //第二次尝试此DelayedOperation成功则返回
    isCompletedByMe = operation.safeTryComplete()
    if (isCompletedByMe)
      return true

    // if it cannot be completed by now and hence is watched, add to the expire queue also
    //将DelayedOperation提交到SystemTimer
    if (!operation.isCompleted) {
      if (timerEnabled)
  ...
  private class Watchers(val key: Any) {
    ...
    //将DelayedOperation添加到oprations队列中
    def watch(t: T) {
      operations.add(t)
    }
    ...
    //负责清理operations队列，将已完成的DelayedOperation从队列中删除，如果队列为空
    //则将Watchers从watchersForKey集合中删除
  ...
  def advanceClock(timeoutMs: Long) {
    ...
    if (estimatedTotalOperations.get - delayed > purgeInterval) {
      ...
      //调用purgeCompleted方法清理完成的DelayedOperation
