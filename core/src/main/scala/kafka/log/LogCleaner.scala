...
class LogCleaner(val config: CleanerConfig,
  ...
  private class CleanerThread(threadId: Int)
    ...
    val cleaner = new Cleaner(id = threadId,
                              //为dirty部分的消息简历key和last_offset的映射关系
                              offsetMap = new SkimpyOffsetMap(memory = math.min(config.dedupeBufferSize / config.numThreads, Int.MaxValue).toInt, 
                                                              hashAlgorithm = config.hashAlgorithm),
                              //指定了读写LogSegment的ByteBuffer大小
                              ioBufferSize = config.ioBufferSize / config.numThreads / 2,
                              //指定的消息得最大长度
                              maxIoBufferSize = config.maxMessageSize,
                              //指定了SkimpyOffsetMap的最大占用比例
    ...
    private def cleanOrSleep() {
      //通过grabFilthiestCompactedLog获得需要进行压缩的日志log
...
private[log] class Cleaner(val id: Int,
  ...
  private[log] def doClean(cleanable: LogToClean, deleteHorizonMs: Long): (Long, CleanerStats) = {
    ...
    //因为activeSegment不参与日志压缩。所以可以确定日志压缩的最大上限是activeSegment.baseOffset
    //activeSegment中对应的第一条消息得offset值
  ...
  private[log] def cleanSegments(log: Log,
    ...
    //创建 .clean后缀的日志文件和索引文件，分组名是第一个LogSegment的baseOffset
    ...
    try {
      ...
      //在这个方法中
      //首先将后缀名为.claen修改为.swap
      //并将cleaned对象加入segments跳表中管理
      //将分组中的logsegment删除
      //最后将文件的.swap后缀名删除
  ...
  private[log] def cleanInto(topicPartition: TopicPartition,
    ...
    while (position < source.log.sizeInBytes) {
      ...
      //如果有需要保留的消息，将消息追加到压缩后的LogSegment中
   ...
   * @param stats Collector for cleaning statistics
   */
  //从firstDirtyOffset开始遍历LogSegment，并填充OffsetMap
  //log中消息是追加的，在填充过程中，key相同，则value会覆盖
  private[log] def buildOffsetMap(log: Log,
    ...
    for (segment <- dirty if !full) {
      //检查LogCleanerManager中记录该分区的状态
      checkDone(log.topicPartition)
      //处理单个LogSegment，将offset添加到OffsetMap中
