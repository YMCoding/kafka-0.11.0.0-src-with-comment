 ...
 */
private[group] class DelayedJoin(coordinator: GroupCoordinator, // GroupCoordinator对象
                                 group: GroupMetadata, // 对应的GroupMetadata
                                 rebalanceTimeout: Long) extends DelayedOperation(rebalanceTimeout) { // 指定了DelayedJoin的到期时长
