 ...
 */
@nonthreadsafe
private[group] class MemberMetadata(val memberId: String, // 消费者id,由GrouoCoordinator分配
                                    val groupId: String, // 记录消费者所在的Consumer Groupid
                                    val clientId: String,
                                    val clientHost: String,
                                    val rebalanceTimeoutMs: Int,
                                    val sessionTimeoutMs: Int, // 心跳超时时间
                                    val protocolType: String,
                                    var supportedProtocols: List[(String, Array[Byte])]) { //对应消费者支持的ParitionAssignor
   ...
   */
  // 从指定候选PartitionAssignor中选择消费者支持的ParitionAssignor的功能
