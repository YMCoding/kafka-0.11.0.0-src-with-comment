...
private[group] class GroupMetadata(val groupId: String, initialState: GroupState = Empty) extends Logging {
  ...
  // 标示当前Consumer group的年代信息，避免过期请求的影响
  var generationId = 0
  // 记录了leader的消费者的memberId
  var leaderId: String = null
  // 记录了当前Consumer Group选择的ParitionAssignor
  ...
  def remove(memberId: String) {
    ...
    if (memberId == leaderId) {
      leaderId = if (members.isEmpty) {
        ...
        // Group leader被删除，则重新选举Group leader
        members.keys.head
      }
    }
  }
  ...
  // 选择合适的ParitionAssignor
  def selectProtocol: String = {
    if (members.isEmpty)
      throw new IllegalStateException("Cannot select protocol for empty group")

    // select the protocol for this group which is supported by all members
    // 所有member都支持的协议 作为候选协议集合
    val candidates = candidateProtocols

    // let each member vote for one of the protocols and choose the one with the most votes
    // 每个member都会通过vote方法进行投票，每个member会为其support protocols集合中第一个候选协议投一票，最终投票最多的作为PartitionAssignor
