...
class GroupMetadataManager(brokerId: Int,
                           ...
                           config: OffsetConfig,
                           replicaManager: ReplicaManager, // Offsets topic和普通的topic一样，在zk中也有leader副本，isr集合，ar集合 也会出现leader副本的迁移，有ReplicaManager管理
                           zkUtils: ZkUtils,
                           time: Time) extends Logging with KafkaMetricsGroup {

  private val compressionType: CompressionType = CompressionType.forId(config.offsetsTopicCompressionCodec.codec)
  // 记录了group在服务端对应的GroupMetadata
  ...
  // 记录了正在加载的offsets topi分区的id
  private val loadingPartitions: mutable.Set[Int] = mutable.Set()

  /* partitions of consumer groups that are assigned, using the same loading partition lock */
  // 记录了已经加载的Offset topi分区的id
  ...
  // 记录了offsets topic的分区数量
  private val groupMetadataTopicPartitionCount = getGroupMetadataTopicPartitionCount

  /* single-thread scheduler to handle offset/group metadata cache loading and unloading */
  // 用于执行group-metadata-manager- 、groupCoordinator迁移等任务
  ...
  def enableMetadataExpiration() {
    ...
    // 启动定时任务。周期的调用cleanupGroupMetadata
  ...
  // 用于检测指定的Consumer group对应的Offsets topic是否处于上述加载过程
              ...
              for (record <- batch.asScala) {
                ...
                GroupMetadataManager.readMessageKey(record.key) match {
                  ...
                  case offsetKey: OffsetKey =>
                    ...
                    // 是删除标记消息，则删除offsetCache中的offsetAndMetadata
                    if (!record.hasValue) {
                      if (isTxnOffsetCommit)
   ...
   */
  // broker成为Offsets topic 分区的follower副本时候，进行清除工作
  def removeGroupsForPartition(offsetsPartition: Int,
    ...
    def removeGroupsAndOffsets() {
      ...
      inLock(partitionLock) {
        ...
        // 从ownedParitions集合中将对应的Offsets topic分区删除，标识当前GroupCoordinator不在管理其对应的Consumer group
        ownedPartitions.remove(offsetsPartition)

        // 遍历groupsCache集合，将改分区对应的GroupMetadata全部清除
  ...
  def cleanupGroupMetadata(deletedTopicPartitions: Option[Seq[TopicPartition]]) {
    ...
    groupMetadataCache.foreach { case (groupId, group) =>
      ...
      // 找到该groupId对应的存储offset消息得Offsets topic分区的id
  ...
  private def getGroupMetadataTopicPartitionCount: Int = {
    // 从zk中获得offset topic这个内部topic的分区信息和副本信息
...
object GroupMetadataManager {
   ...
   */
  // 用于创建记录GroupMetadata的消息得key，由group、topic、Paritionid组成
