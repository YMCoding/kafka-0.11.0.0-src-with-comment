...
object PreferredReplicaLeaderElectionCommand extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // 将指定分区写入到zk中的 admin/preferred_replica_election节点中
...
class PreferredReplicaLeaderElectionCommand(zkUtils: ZkUtils, partitionsFromUser: scala.collection.Set[TopicAndPartition]) {
  // 检测指定的topic是否包含指定的分区，调用writePreferredReplicaElectionData方法将需要优先副本选举的分区信息写入zk
