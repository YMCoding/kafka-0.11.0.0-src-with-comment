...
class TopicDeletionManager(controller: KafkaController, eventManager: ControllerEventManager) extends Logging {
  ...
  val partitionStateMachine = controller.partitionStateMachine //管理分区状态的状态机
  val replicaStateMachine = controller.replicaStateMachine //管理副本状态的状态机
  val isDeleteTopicEnabled = controller.config.deleteTopicEnable
  val topicsToBeDeleted = mutable.Set.empty[String] //用于记录将要被删除的topic集合
  val partitionsToBeDeleted = mutable.Set.empty[TopicAndPartition] //用于记录将要被删除的分区集合
  val topicsIneligibleForDeletion = mutable.Set.empty[String] //用于记录不可删除的topic集合
  ...
  def enqueueTopicsForDeletion(topics: Set[String]) {
    if(isDeleteTopicEnabled) {
      //待删除的topic放入topicsToBeDeleted
      topicsToBeDeleted ++= topics
      //待删除的topic的分区集合访入partitionsToBeDeleted
  ...
  def resumeDeletionForTopics(topics: Set[String] = Set.empty) {
    if(isDeleteTopicEnabled) {
      //从指定topic集合与待删除Topic集合的交集，如果这个交集不为空，将交集中的topic从指定的topicsIneligibleForDeletion中删除
  ...
  def failReplicaDeletion(replicas: Set[PartitionAndReplica]) {
    if(isDeleteTopicEnabled) {
      ...
      if(replicasThatFailedToDelete.nonEmpty) {
        ...
        //将异常副本转换为ReplicaDeletionIneligible状态
  ...
  def completeReplicaDeletion(replicas: Set[PartitionAndReplica]) {
    ...
    //将成功删除的副本状态转换为successfullyDeletedReplicas
  ...
  private def isTopicEligibleForDeletion(topic: String): Boolean = {
    topicsToBeDeleted.contains(topic) && //当前topic没有完全完成删除操作
      (!isTopicDeletionInProgress(topic) && //topic删除操作还没开始
        !isTopicIneligibleForDeletion(topic)) //topic没有被标记为不可删除
  }
   ...
   */
  //将处于ReplicaDeletionIneligible的状态转换成OfflineReplica
  ...
  private def onTopicDeletion(topics: Set[String]) {
    ...
    //向可用Broker发送UpdateMetadataRequest
  ...
  private def startReplicaDeletion(replicasForTopicsToBeDeleted: Set[PartitionAndReplica]) {
    replicasForTopicsToBeDeleted.groupBy(_.topic).keys.foreach { topic =>
      ...
      //将不可用副本转换为ReplicaDeletionIneligible
      replicaStateMachine.handleStateChanges(deadReplicasForTopic, ReplicaDeletionIneligible)

      // send stop replica to all followers that are not in the OfflineReplica state so they stop sending fetch requests to the leader
      // 将待删除副本状态转换为OfflineReplica
      replicaStateMachine.handleStateChanges(replicasForDeletionRetry, OfflineReplica)
      debug("Deletion started for replicas %s".format(replicasForDeletionRetry.mkString(",")))

      // 将待删除的副本转换为ReplicaDeletetionStarted状态
  ...
  private def resumeDeletions(): Unit = {
    ...
    topicsQueuedForDeletion.foreach { topic =>
      ...
      //检测所有的副本是否处于ReplcaDeletionSuccessful状态
      if(controller.replicaStateMachine.areAllReplicasForTopicDeleted(topic)) {

        // clear up all state for this topic from controller cache and zookeeper
        // 清空所有副本和partition的状态，清空ControllerContext和zk中此topic信息
        completeDeleteTopic(topic)
        info("Deletion of topic %s successfully completed".format(topic))
      } else {
        //如果任意一个副本处于ReplicaDeletionStarted。则继续等待
        if(controller.replicaStateMachine.isAtLeastOneReplicaInDeletionStartedState(topic)) {
          ...
          //任意一副本处于ReplicaDeleteIneligble。则重置为OfflineReplca后U重试
