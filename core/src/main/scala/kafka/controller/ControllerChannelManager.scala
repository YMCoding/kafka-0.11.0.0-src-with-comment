...
class ControllerChannelManager(controllerContext: ControllerContext, config: KafkaConfig, time: Time, metrics: Metrics,
  ...
  protected val brokerStateInfo = new HashMap[Int, ControllerBrokerStateInfo]//用来管理集群中每个broker对应的ControllerBrokerStateInfo对象
  ...
  //添加每个broker对应的ControllerBrokerStateInfo
  ...
  //移除Broker对应的ControllerBrokerStateInfo对象
  ...
  override def doWork(): Unit = {
    ...
    try {
      ...
      while (isRunning.get() && !isSendSuccessful) {
        //当broker宕机后，会触发zk的监听器调用removeBroker方法将蛋清的线程停止
...
class ControllerBrokerRequestBatch(controller: KafkaController) extends  Logging {
  ...
  //记录了发往指定Broker的leaderAndIsrRequest所需信息
  ...
  //记录了发往指定broker的UpdateMetadaRequest集合
  ...
  //向LeaderAndIserRequestMap集合中添加要发送的LeaderAndIsrRequest所需的数据
...
//表示与一个Broker连接的各种信息
case class ControllerBrokerStateInfo(networkClient: NetworkClient, //维护contoller对应broker通信的网络连接
                                     brokerNode: Node,//维护了对应的broker的网络位置信息，记录了Broker的ip、port等
                                     messageQueue: BlockingQueue[QueueItem],//缓冲队列，存放了发往对应的broker请求
                                     requestSendThread: RequestSendThread,//用于请求发送的线程
