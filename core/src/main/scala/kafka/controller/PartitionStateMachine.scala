 ...
 *                          Valid previous states are NewPartition/OfflinePartition
  *   成功选出副本，但是leader副本发生宕机
 * 4. OfflinePartition    : If, after successful leader election, the leader for partition dies, then the partition
 *                          moves to the OfflinePartition state. Valid previous states are NewPartition/OnlinePartition
 */
//用于维护分区状态的状态机
//分区状态通过PartitonState接口定义的
class PartitionStateMachine(controller: KafkaController) extends Logging {
  private val controllerContext = controller.controllerContext //用于维护KafkaController上下文信息
  private val controllerId = controller.config.brokerId
  private val zkUtils = controllerContext.zkUtils //zk客户端
  private val partitionState: mutable.Map[TopicAndPartition, PartitionState] = mutable.Map.empty //记录了每个分区对应的Partiton状态
  private val brokerRequestBatch = new ControllerBrokerRequestBatch(controller) //用于向指定的Broker批量发送请求
  private val noOpPartitionLeaderSelector = new NoOpLeaderSelector(controllerContext) //默认的Leader副本选举类器
   ...
   */
  //控制这PartitionState的转换
  private def handleStateChange(topic: String, partition: Int, targetState: PartitionState,
                                leaderSelector: PartitionLeaderSelector,
                                callbacks: Callbacks) {
    val topicAndPartition = TopicAndPartition(topic, partition)
    //从PartitionState集合中获取分区的状态 ，没有对应的状态，则初始化为NonExistentPartition
    val currState = partitionState.getOrElseUpdate(topicAndPartition, NonExistentPartition)
    try {
      assertValidTransition(topicAndPartition, targetState)

      //转换之前，根据targerState检查分区的前置状态是否合法
      targetState match {
          //将分区状态甚至为NewPartition
        case NewPartition =>
  ...
  private def initializePartitionState() {
    ...
    for (topicPartition <- controllerContext.partitionReplicaAssignment.keys) {
      ...
      //partitionLeadershipInfo 中记录了Leader副本信息和Isr集合信息
   ...
   */
  //由NewPartition切换为OnlinePartition
  private def initializeLeaderAndIsrForPartition(topicAndPartition: TopicAndPartition) = {
    ...
    liveAssignedReplicas.headOption match {
      ...
      case Some(leader) =>
        ...
        //创建leaderIsrAndControllerEpoch对象，其中的isr集合是可以的ar集合
        ...
        try {
          ...
          // GC pause
          //更新partitionAndControllerEpoch中的记录
          controllerContext.partitionLeadershipInfo.put(topicAndPartition, leaderIsrAndControllerEpoch)
          //添加LeaderAdnIsrRequest待发送
   ...
   */
  //OfflinePartition切换为OnlinePartition
  def electLeaderForPartition(topic: String, partition: Int, leaderSelector: PartitionLeaderSelector) {
    ...
    try {
      ...
      while(!zookeeperPathUpdateSucceeded) {
        //从zk中获取当前分区的Leader副本，Isr集合，zkVersion等信息
