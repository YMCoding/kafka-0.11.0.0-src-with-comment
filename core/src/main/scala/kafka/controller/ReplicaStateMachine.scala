 ...
 * transitions to move the replica to another legal state. The different states that a replica can be in are -
  *   创建新topic或进行副本的重新分配，此状态的副本只能成为follower副本
 * 1. NewReplica        : The controller can create new replicas during partition reassignment. In this state, a
 *                        replica can only get become follower state change request.  Valid previous
 *                        state is NonExistentReplica
  *   副本开始正常工作处于的状态。可以成为leader也可以成为follower
 ...
 */
//Controller用于维护副本的状态机
class ReplicaStateMachine(controller: KafkaController) extends Logging {
  private val controllerContext = controller.controllerContext
  private val controllerId = controller.config.brokerId
  private val zkUtils = controllerContext.zkUtils
  private val replicaState: mutable.Map[PartitionAndReplica, ReplicaState] = mutable.Map.empty //记录每个副本对应的ReplicaState状态
    ...
    try {
      ...
      targetState match {
        case NewReplica =>
          ...
          //从zk中获取分区的Leader副本，isr信息
          val leaderIsrAndControllerEpochOpt = ReplicationUtils.getLeaderIsrAndEpochForPartition(zkUtils, topic, partition)
          leaderIsrAndControllerEpochOpt match {
            case Some(leaderIsrAndControllerEpoch) =>
              //处于NewReplica状态的副本不可能是Leader
              if(leaderIsrAndControllerEpoch.leaderAndIsr.leader == replicaId)
                throw new StateChangeFailedException("Replica %d for partition %s cannot be moved to NewReplica"
                  .format(replicaId, topicAndPartition) + "state as it is being requested to become leader")

              //向改副本发送LeaderAndIsrRequest。并发送UpdateMetadataRequest给所有可用的Broker
          ...
          stateChangeLogger.trace("Controller %d epoch %d changed state of replica %d for partition %s from %s to %s"
                                    .format(controllerId, controller.epoch, replicaId, topicAndPartition, currState,
                                             targetState))
          //更新副本的状态为ReplicaDeletionStarted
        case ReplicaDeletionStarted =>
          ...
          //更新副本的状态为ReplicaDeletionIneligible
        ...
        case OnlineReplica =>
          ...
          replicaState(partitionAndReplica) match {
            ...
            case _ =>
              ...
              controllerContext.partitionLeadershipInfo.get(topicAndPartition) match {
                case Some(leaderIsrAndControllerEpoch) =>
                  //存在leader，向该副本发送LeaderAndIsrRequest，并把UpdateMetadataRequest给所有可用的Broker
          ...
          //更新副本的状态为OnlineReplica
          replicaState.put(partitionAndReplica, OnlineReplica)
        case OfflineReplica =>
          //向副本发送StopReplcaRequest
