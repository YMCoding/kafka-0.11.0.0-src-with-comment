...
class SimpleAclAuthorizer extends Authorizer with Logging {
  ...
  private var superUsers = Set.empty[KafkaPrincipal] // 用于记录配置文件中指定的超级用户信息
  private var shouldAllowEveryoneIfNoAclIsFound = false // 为false标示，如果一个资源ACLs中没有相关记录时候，除了超级用户，任何用户都不能访问
  private var zkUtils: ZkUtils = null
  private var aclChangeListener: ZkNodeChangeNotificationListener = null // zk上的监听器

  private val aclCache = new scala.collection.mutable.HashMap[Resource, VersionedAcls] // acl在内存中的缓存
  ...
  override def configure(javaConfigs: util.Map[String, _]) {
    ...
    // 根据配置，初始化shouldAllowEveryoneIfNoAclIsFound
  ...
  // 遍历zk kafka-acl节点记录。加载到aclCache集合
  private def loadCache()  {
    // 加锁
    inWriteLock(lock) {
      // 获取kafka-acl-子节点集合，此集合标示资源的类型，例如topic，cluster
