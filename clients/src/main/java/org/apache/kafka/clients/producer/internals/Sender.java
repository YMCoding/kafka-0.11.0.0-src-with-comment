...
public class Sender implements Runnable {
    ...
    public void run() {
        ...
        while (running) {
            try {
                //处理消息的发送,调用NetWorkClient中的poll方法,最终调用KafkaChannel的write方法
        ...
        // 如果不是强制关闭
        // 且消息累加器accumulator尚有消息未发送，
        // 或者客户端client尚有正在处理（in-flight）的请求，
        // 处理尚未发送完的消息的发送；
        ...
        // 如果是强制关闭
        // 调用消息累加器accumulator的abortIncompleteBatches()，放弃未处理完的请求；
        if (forceClose) {
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        if (batches.isEmpty())
            return;
        //MemoryRecords就是存储数据的buffer,传递给ClientRequest
        Map<TopicPartition, MemoryRecords> produceRecordsByPartition = new HashMap<>(batches.size());
        //传给callback,调用completeBatch处理bath
